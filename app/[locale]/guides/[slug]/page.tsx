...
export default function GuidebookDetailPage({
  ...
  const allGuidesArr = guides
  const currentIdx = allGuidesArr.findIndex((g) => g.slug === slug)
  const prev = currentIdx > 0 ? allGuidesArr[currentIdx - 1] : null
  const next = currentIdx < allGuidesArr.length - 1 ? allGuidesArr[currentIdx + 1] : null
  ...
  return (
    <>
      <div className="flex gap-0">
        <GuidebookSidebar locale={locale} currentSlug={slug} />
        ...
        <div className="flex-1 min-w-0 px-5 py-4">
          ...
          <div className="flex items-center gap-1.5 text-xs mb-4" style={{ color: 'var(--text-secondary)' }}>
            ...
            <span>/</span>
            <Link href={`/${locale}/guides`} className="hover:opacity-80">
              {locale === 'th' ? 'คู่มือ' : 'Guidebook'}
            </Link>
            <span>/</span>
          ...
          <GuideNav prev={prev} next={next} locale={locale} basePath="guides" />
