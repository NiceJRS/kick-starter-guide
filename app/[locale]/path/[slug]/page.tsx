...
const pathGuides = guides.filter((g) => g.showInPath).sort((a, b) => a.streamerPathOrder - b.streamerPathOrder)
...
export default function PathDetailPage({
  ...
  if (!guide || !guide.showInPath) notFound()

  const mdxDoc = allGuides.find((g) => g.slug === slug && g.locale === locale)

  const currentIdx = pathGuides.findIndex((g) => g.slug === slug)
  const prev = currentIdx > 0 ? pathGuides[currentIdx - 1] : null
  const next = currentIdx < pathGuides.length - 1 ? pathGuides[currentIdx + 1] : null
  ...
  return (
    <>
      <div className="flex gap-6">
        <GuideSidebar locale={locale} currentSlug={slug} completed={completed} basePath="path" />
        ...
        <div className="flex-1 min-w-0">
          ...
          <GuideNav prev={prev} next={next} locale={locale} basePath="path" />
      ...
      {/* Mobile chapter drawer — only path guides */}
      {mobileNavOpen && (
